...
amctl agent metrics "$AGENT" --project "$PROJ" --env default --since 1h --json \
  | jq '{cpu:(.data.cpuUsage|length), mem:(.data.memoryUsage|length), latestMem:.data.memoryUsage[-1]}'
...
| Build `Completed`, only init-container otel-tracing logs visible, `cpu:0 mem:0`, 10+ min after build | Pod never reached app layer. CLI can't tell you more. Escalate to cluster (`kubectl describe pod`, `kubectl get events`). |
| `cpu > 0` AND `mem == 0` | Crash-loop signature: alive at startup, dead before mem scrape. |
| Build `Completed`, app logs healthy, `mem > 0`, `traces count: 0` | Pod up, no traffic, or auto-instrumentation off. Check `--no-auto-instrumentation` wasn't set at create. |
